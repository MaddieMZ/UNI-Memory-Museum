...
// Variable para controlar la rotaci車n del objeto
GLfloat rotationAngle = 0.0;

// Funci車n para cargar un modelo OBJ utilizando Assimp
...
// Funci車n de inicializaci車n de OpenGL
void init() {
...
// Funci車n de renderizado
void render() {
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    // Configurar la c芍mara
...
// Funci車n de manejo de movimiento del rat車n
void mouseMotion(int x, int y) {
    // Rotar el objeto en funci車n del movimiento del rat車n
    rotationAngle += (x - glutGet(GLUT_WINDOW_WIDTH) / 2) * 0.1;
    glutPostRedisplay();
}
// Funci車n de manejo de eventos del rat車n para rotar la c芍mara al hacer clic y arrastrar
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        // Guardar la posici車n inicial del clic
        glutMotionFunc(mouseMotion);
    }
    else if (button == GLUT_LEFT_BUTTON && state == GLUT_UP) {
        // Detener la rotaci車n cuando se libera el clic
...
int main(int argc, char** argv) {
    ...
    // Registrar funciones de manejo de eventos del rat車n
    glutMouseFunc(mouse);
    glutMotionFunc(nullptr);
